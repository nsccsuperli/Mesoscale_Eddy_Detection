...
"""
修改
 1、Image.open,将三通道改成单通道 修改后        if self.transform is not None: 出错，原因是原来是batchsize，chanel，height，width 四通道现在是三通道
                                                img = self.transform(img) 最终错误来源于init文件中的 standard_transforms.Normalize(*mean_std)
...
def add_items(items, aug_items, cities, img_path, mask_path, edge_path, mode, maxSkip):
...
class CityScapes(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_path, mask_path,edge_path = self.imgs[index]

        img, mask, edge = Image.open(img_path).convert('L'), Image.open(mask_path),Image.open(edge_path).convert('L')
        ...
        edge = Image.fromarray(edge_copy.astype(np.uint8))

        # Image Transformations
        # img_1 = self.transform(img)
        # mask_1 = self.transform(mask)

        if self.joint_transform is not None:
            img, mask,edge = self.joint_transform(img, mask,edge)
    ...
    #     self.dump_images =True
        if self.dump_images:
            ...
            out_img_fn = os.path.join(outdir, img_name + '_edge.png')
            out_msk_fn = os.path.join(outdir, img_name + '_mask.png')
            mask_img = colorize_mask(np.array(mask))
            # eddy_img = colorize_mask(np.array(edgemap))
            # eddy_img.save(out_img_fn)
            # img.save(out_img_fn)
